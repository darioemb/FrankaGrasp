...
#define SOFTSTUFFHEIGHT 0.035		//new 30.1.2012 => 1.5 cm for inaccuracy
...
public:
	...
	ros::Publisher position_basket_pub;
	...
	void filter_pc(pcl::PointCloud<pcl::PointXYZ>& pcl_cloud_merged,
			       /*bool pub_basket_position=false, float x_min=0.6, float x_max=1.3,
			        * float y_min=-0.8, float y_max=0.1, float z_min=0.02, float z_max = 0.37);*/
			       bool pub_basket_position=false, float x_min=-0.5, float x_max=0.3,
			       float y_min=-0.1, float y_max=0.33, float z_min=0.01, float z_max = 0.37);
	// filters outliers and points outside defined region, XYZRC point cloud
	void filter_pc_rc(pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_cloud_merged,
			          pcl::PointCloud<PointXYZRC>& final_cloud,
			          bool pub_basket_position=true, float x_min=0.6, float x_max=1.3,
			          float y_min=-0.8, float y_max=0.1, float z_min=0.04, float z_max = 0.37);
	void downsampling_pc(pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_cloud_merged);
	...
	// receives point clouds from camera #2
	void pc_cam2_callback(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr& pcl_in);
