...
for line in dat.readlines():
  ...
  if (s[0] != "Name"):
      ...
      if (len(words) != 0):
        if (words[0] == 'v'):
          ...
          if float(words[1]) > mx_x:
            mx_x = float(words[1])
          if float(words[3]) < mn_y:
            mn_y = float(words[3])
          if float(words[3]) > mx_y:
            mx_y = float(words[3])
          if float(words[2]) < mn_z:
            mn_z = float(words[2])
          if float(words[2]) > mx_z:
            mx_z = float(words[2])
    ...
    f = open('env/%s.obj' % s[0], 'r')
    nf = open('env/%s-scaled.obj' % s[0], 'w')
    print 'env/%s-scaled.obj' % s[0]
    while True:
        l = f.readline()
        if len(l) == 0:
          break
        ...
        if ((words[0] == 'vn') or (words[0] == 'v')):
          ...
          if isnum(words[3]):
            scaledy = (float(words[3])-mn_y) * (y_dat/(mx_y - mn_y)) -(y_dat/2)
          if isnum(words[2]):
            scaledz = (float(words[2])-mn_z) * (z_dat/(mx_z - mn_z))
#            scaledz = (mx_z-float(words[2])) * (z_dat/(mx_z - mn_z))
