...
from openravepy.openravepy_int import RaveCreateModule, RaveCreateTrajectory, IkParameterization, IkParameterizationType, RaveFindDatabaseFile, RaveDestroy, Environment, Robot, KinBody, DOFAffine, CollisionReport, RaveCreateCollisionChecker, quatRotateDirection, rotationMatrixFromQuat
...
class GraspingModel(DatabaseGenerator):
    ...
    def __init__(self,robot,target,demo_interactive, maxvelmult=None):  #david
        DatabaseGenerator.__init__(self,robot=robot)
        assert(target is not None)
        self.target = target
        self.demo_interactive = demo_interactive    #david
        self.manipdir = robot.GetActiveManipulator().GetDirection() #david
        self.grasps = []
...
#                 graspingnoise = 0.01 # 0.01m of noise
    ...
    #david begin
    def generate2(self,maxGoodGrasps=1000,preshapes=None,standoffs=None,rolls=None,approachrays=None, graspingnoise=None,updateenv=True,forceclosure=False,forceclosurethreshold=1e-9,checkgraspfn=None,disableallbodies=True):
        ...
        numGoodGrasps = 0 #count number of good grasps already found
        if approachrays is None:
        ...
        approachgraphs = [self.env.plot3(points=gapproachrays[:,0:3],pointsize=5,colors=array((1,0,0))),
                          self.env.drawlinelist(points=reshape(c_[gapproachrays[:,0:3],gapproachrays[:,0:3]+0.35*gapproachrays[:,3:6]],(2*N,3)),linewidth=4,colors=array((1,0,0,1)))]
        print gapproachrays
        
        if (self.demo_interactive):
            raw_input('Showing Approach Vectors. Press Enter to continue')
        else:
            time.sleep(0.5)
        contactgraph = None
        ...
        try:
            with self.GripperVisibility(self.manip):
                ...
                for approachray in approachrays:
                    if numGoodGrasps >= maxGoodGrasps:
                        break 
                    for roll,preshape,standoff in iterproduct(rolls,preshapes,standoffs):
                        print 'grasp %d/%d'%(counter+1,totalgrasps),'preshape:',preshape,' numGoodGrasp found: ',numGoodGrasps
                        counter += 1
                        ...
                        grasp[self.graspindices.get('imanipulatordirection')] = self.manipdir   #david
                        self.env.SetDebugLevel(1) #david 23.1.2012
                        try:
                            ...
                            continue
                        Tlocalgrasp = eye(4)
                        #self.env.SetDebugLevel(DebugLevel.Info) #david 23.1.2012
                        with self.robot:
                            ...
                            if not forceclosure or mindist >= forceclosurethreshold:
                                if checkgraspfn is None or checkgraspfn(contacts,finalconfig,grasp,{'mindist':mindist,'volume':volume}):
                                    ...
                                    #print "finalconfig: ", finalconfig
                                    #print "mindist: ", mindist
                                    #print "volume: ",volume
                                    #raw_input("see above for contacts")
                                    #david end
...
#                     time.sleep(1)
    ...
    def runGrasp(self,grasp,graspingnoise=None,translate=True,forceclosure=False,translationstepmult=None,finestep=None):
        ...
        with self.robot: # lock the environment and save the robot state
            ...
            #return self.grasper.Grasp(direction=grasp[self.graspindices.get('igraspdir')], roll=grasp[self.graspindices.get('igrasproll')], position=grasp[self.graspindices.get('igrasppos')], standoff=grasp[self.graspindices.get('igraspstandoff')], manipulatordirection=array([self.manip.GetDirection()]), target=self.target,graspingnoise = graspingnoise, forceclosure=forceclosure, execute=False, outputfinal=True,translationstepmult=translationstepmult,finestep=finestep)
