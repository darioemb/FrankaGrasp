...
public:
	...
	CCalc_Grasppoints(ros::NodeHandle nh)
	{
		...
		graspval_th = 100;					//treshhold if grasp hypothesis should be returned (in funtion - so programm internal) (for top result of one loop run)
		graspval_top = 119;
		graspval_max_diff_for_pub = 99;		//if the value of grasps is more that graspval_max_diff_for_pub lower than optimal value graspval_top, nothing gets published (for top result of one whole roll run)
...
{
	ROS_INFO("\nFrom calc_grasppoints: box_position received");
	int start = 0, end = 0;
...
//loop goes through all rools and tilts and executes neccessary mehtods for calculation of gps
void CCalc_Grasppoints::loop_control(pcl::PointCloud<pcl::PointXYZ> pcl_cloud_in)
{
   ...
   double timedif;
	...
	for (int tilt = 0; tilt < TILT_STEPS; tilt++)
	{
		for (int roll = 0; roll < ROLL_MAX_DEGREE/ROLL_STEPS_DEGREE; roll++)
		{
		...
		//david new 7.2.: always send best gh after all rolls where tried which is not worse than this->graspval_max_diff_for_pub from top grasp possible
...
void CCalc_Grasppoints::predict_bestgp_withsvm(bool svm_with_probability){
	...
	int i = system("./libsvm-3.12/svm-scale -r data/range21062012_allfeatures /tmp/features.txt > /tmp/features.txt.scale");
	cout << "\n\n i (scale): " << i << endl;

	if (!svm_with_probability){
		//predict graspint points
		/*	trained with 1000 manual examples (500-500) examples*/
    	//i = system("/usr/lib/libsvm/libsvm-3.1/svm-predict /tmp/features.txt.scale /usr/lib/libsvm/libsvm-3.1/tools/mixedmanual_features_1000_rand_f300.scale.txt.model /tmp/output_calc_gp.txt");

		//	trained with all manual examples
		//i = system("/usr/lib/libsvm/libsvm-3.1/svm-predict /tmp/features.txt.scale /usr/lib/libsvm/libsvm-3.1/tools/allmanualfeatures.txt.scale.model /tmp/output_calc_gp.txt");

		//	trained with all examples
		i = system("./libsvm-3.12/svm-predict /tmp/features.txt.scale data/all_features.txt.scale.model /tmp/output_calc_gp.txt");
...
void CCalc_Grasppoints::show_predicted_gps(int nr_roll, int tilt, bool svm_with_probability){
		...
		for (int col = 0; col < WIDTH; col++){
			if ( graspsgrid[row][col] < 0 ){
				...
				graspseval[row][col] =
						...
						w1*graspsgrid[row][col-4] + w1*graspsgrid[row][col-3] + w3*graspsgrid[row][col-2] + w4*graspsgrid[row][col-1] + w5*graspsgrid[row][col] + w4*graspsgrid[row][col+1] + w3*graspsgrid[row][col+2] + w1*graspsgrid[row][col+3] +w1*graspsgrid[row][col+4] + //row
						w2*graspsgrid[row+1][col-2]+w3*graspsgrid[row+1][col-1]+w4*graspsgrid[row+1][col]+w3*graspsgrid[row+1][col+1]+w2*graspsgrid[row+1][col+2]+ //row +1
						w1*graspsgrid[row+2][col-2]+w2*graspsgrid[row+2][col-1]+w3*graspsgrid[row+2][col]+w2*graspsgrid[row+2][col+1]+w1*graspsgrid[row+2][col+2]; //row +2
	...
	if ((topval_gp_all > this->graspval_th) and(!this->return_only_best_gp) ){
		...
		cout << "\n\n NO GRASP POINT FOUND above treshold for one loop step!!! \n";
}
...
void CCalc_Grasppoints::transform_gp_in_wcs_and_publish(int id_row_top_all, int id_col_top_all,int nr_roll_top_all, int nr_tilt_top_all, int scaled_gp_eval){
    ...
    //for cornell demo output was changed to a single point:
    float outputangle = -angle;	//in RAD!!
    cout << "PI: " << PI << "  2*PI: " << 2*PI << "   outputangle: " << outputangle << endl;
    if (outputangle < 0)
    	cout << " \n\n outputangle kleiner 0 => hab irgendwo denkfehler!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n" << endl;;
    ss << (gp1_wcs[0]+gp2_wcs[0])/2 << " " << (gp1_wcs[1]+gp2_wcs[1])/2 << " " << (gp1_wcs[2]+gp2_wcs[2])/2 << " " << outputangle;

    msgStrPoints.data = ss.str();
	//pubGraspPoints.publish(msgStrPoints);
	cout << "\n scaled_gp_eval: " << scaled_gp_eval << endl;
	cout << "\n grasp points in object coordinate system: " << ss.str() << endl;
	cout << "\n alpha published: " << outputangle << endl;
