...
#                 graspingnoise = 0.01 # 0.01m of noise
    ...
    #david: inserted on 24th of August 2012 - begin 
    def generate2(self,maxGoodGrasps=1000,preshapes=None,standoffs=None,rolls=None,approachrays=None, graspingnoise=None,updateenv=True,forceclosure=False,forceclosurethreshold=1e-9,checkgraspfn=None,disableallbodies=True):
        ...
        counter = 0
        self.grasps = []
        self.manipdir = self.robot.GetActiveManipulator().GetDirection() #david
        statesaver = self.robot.CreateRobotStateSaver()
        bodies = [(b,b.IsEnabled()) for b in self.env.GetBodies() if b != self.robot and b != self.target]
        if disableallbodies:
            for b in bodies:
            ...
            with self.GripperVisibility(self.manip):
                ...
                for approachray in approachrays:
                    ...
                    for roll,preshape,standoff in iterproduct(rolls,preshapes,standoffs):
                        ...
                        grasp[self.graspindices.get('igraspstandoff')] = standoff
                        grasp[self.graspindices.get('igrasppreshape')] = preshape
                        grasp[self.graspindices.get('imanipulatordirection')] = self.manipdir
