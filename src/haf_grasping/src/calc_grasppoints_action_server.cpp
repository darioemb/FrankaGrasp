...
public:
	...
	CCalc_Grasppoints(string name) :
	    ...
	    action_name_(name)
	{
		// this->pub_haf_point = nh_.advertise<geometry_msgs::Pose>("/pub_haf_point",1);
...
void CCalc_Grasppoints::loop_control(pcl::PointCloud<pcl::PointXYZ> pcl_cloud_in)
{
	...
	if(success)	// ActionServer: return grasp representation (overall best grasp)
	{
		...
		// pose.position.x = this->gp_result.averagedGraspPoint.x;
		// pose.position.y = this->gp_result.averagedGraspPoint.y;
		// pose.position.z = this->gp_result.averagedGraspPoint.z;
		// pose.orientation.w = this->gp_result.roll;
		
		// pub_haf_point.publish(pose);
